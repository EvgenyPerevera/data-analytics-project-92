-- считает общее количество покупателей из таблицы customers, присваивает колонке псевдоним customers_count
SELECT
...
SELECT
    CONCAT(e.first_name, ' ', e.last_name) AS seller, --берем имя и фамилию из таблицы employees, объединяем в одну строку с пробелом с помощью CONCAT, присваиваем колонке псевдоним seller
    FLOOR(AVG(p.price * s.quantity)) AS average_income --считаем среднюю выручку с одной сделки продавца, округляем до целых чисел (в меньшую сторону), присваиваем колонке псевдоним average_income
FROM sales s
JOIN employees e 
